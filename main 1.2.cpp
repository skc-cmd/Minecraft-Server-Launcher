...
std::vector<ModLoaderInfo> getSupportedLoaders() {
    ...
    // ===== 以下为预留接口，暂不启用 =====
    // 如需启用，将 supported 改为 true 并填写完整信息即可

    // Forge（预留）
    // ModLoaderInfo forge;
    // forge.name = "Forge";
    // forge.id = "forge";
    // forge.installerUrl = "https://...";
    // forge.installerJar = "forge-installer.jar";
    // forge.mainClass = "";
    // forge.generatedJar = "forge-server-launch.jar";
    // forge.supported = false;
    // loaders.push_back(forge);

    // Quilt（预留）
    // ModLoaderInfo quilt;
    // quilt.name = "Quilt";
    // quilt.id = "quilt";
    // quilt.installerUrl = "https://...";
    // quilt.installerJar = "quilt-installer.jar";
    // quilt.mainClass = "";
    // quilt.generatedJar = "quilt-server-launch.jar";
    // quilt.supported = false;
    // loaders.push_back(quilt);

    // NeoForge（预留）
    // ModLoaderInfo neoforge;
    // neoforge.name = "NeoForge";
    // neoforge.id = "neoforge";
    // neoforge.installerUrl = "https://...";
    // neoforge.installerJar = "neoforge-installer.jar";
    // neoforge.mainClass = "";
    // neoforge.generatedJar = "neoforge-server-launch.jar";
    // neoforge.supported = false;
    // loaders.push_back(neoforge);
...
void startServer() {
    ...
    if (!eulaAccepted) {
        showSeparator();
        setConsoleColor(COLOR_YELLOW);
        std::cout << "  需要同意以下条款才能启动服务器" << std::endl;
        resetConsoleColor();
        showSeparator();
        std::cout << std::endl;

        setConsoleColor(COLOR_YELLOW);
        std::cout << "【免责声明与用户协议】" << std::endl;
        resetConsoleColor();
        std::cout << std::endl;
        std::cout << "1. 本程序为第三方启动器，并非Minecraft官方产品。" << std::endl;
        std::cout << "   未经Mojang Studios或Microsoft批准，也不与之关联。" << std::endl;
        ...
        char confirm;
        std::cout << "你是否同意以上条款和Minecraft EULA? (y/n): ";
        std::cin >> confirm;

        if (confirm != 'y' && confirm != 'Y') {
            showInfo("用户拒绝同意条款，取消启动");
            std::cout << "\n按任意键继续...";
            std::cin.ignore();
            std::cin.get();
            return;
...
void showMenu() {
    ...
    std::cout << "  4. 配置版本的配置文件 (server.properties)" << std::endl;
    std::cout << "  5. 安装服务器自动配置" << std::endl;
    std::cout << "  6. 启动选项配置" << std::endl;
    std::cout << "  7. 安装模组加载器" << std::endl;
    std::cout << "  8. 列出已安装的服务器" << std::endl;
    std::cout << "  9. 查看可用版本" << std::endl;
