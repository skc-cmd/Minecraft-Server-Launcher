...
void scanPlugins() {
    ...
    do {
        ...
        // 调试：检测到文件夹
        std::cout << "[DEBUG] 找到文件夹: " << folderName << std::endl;

        if (!fileExists(configPath)) {
            std::cout << "[DEBUG] 跳过: " << folderName << " (config.json 不存在)" << std::endl;
            continue;
        }
        std::cout << "[DEBUG] " << folderName << " 的 config.json 存在" << std::endl;

        std::ifstream file(configPath);
        if (!file.is_open()) {
            std::cout << "[DEBUG] 跳过: " << folderName << " (无法打开 config.json)" << std::endl;
            continue;
        }
        ...
        std::cout << "[DEBUG] config.json 内容: " << content << std::endl;
        ...
        try {
            ...
            std::cout << "[DEBUG] exe 路径: " << plugin.exePath << std::endl;
            std::cout << "[DEBUG] exe 是否存在: " << (fileExists(plugin.exePath) ? "是" : "否") << std::endl;

            if (fileExists(plugin.exePath)) {
                g_plugins.push_back(plugin);
                std::cout << "[DEBUG] 插件加载成功: " << plugin.name << std::endl;
            }
            else {
                std::cout << "[DEBUG] 插件加载失败: exe 不存在" << std::endl;
            }

        }
        catch (const json::parse_error& e) {
            std::cout << "[DEBUG] JSON 解析失败: " << folderName << " - " << e.what() << std::endl;
            showDebug("解析插件配置失败: " + folderName);
        }
        catch (const std::exception& e) {
            std::cout << "[DEBUG] 异常: " << folderName << " - " << e.what() << std::endl;
    ...
    std::cout << "[DEBUG] 扫描完成，共找到 " << g_plugins.size() << " 个插件" << std::endl;
...
int main() {
    ...
    // ===== 扫描插件 =====
    scanPlugins();
    std::cout << "[DEBUG] 扫描完成，插件数量: " << g_plugins.size() << std::endl;
    int choice;
    while (true) {
        showMenu();
        std::cin >> choice;
